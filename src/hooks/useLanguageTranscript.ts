...
/**
 * WHY: only one instance may drive the recogniser. Screen remounts can leave
 * an old instance alive whose async cleanup has not finished - three of them
 * aborting and restarting each other was eating the transcript and made
 * start/stop/resume behave differently on every attempt. Newest mount wins.
 */
...
export function useLanguageTranscript({
  ...
  const startListening = useCallback(async (fromRestart = false) => {
    ...
    // Claim the recogniser so a stale instance cannot restart underneath us.
