...
export function useIdeaCapture() {
  ...
  // Android 11 (API < 33): preferSavedAudio=true → no live text (OPPO A51 bug path).
  const captureMode = resolveCaptureMode(preferSavedAudio);
  const audioOnly = captureMode === 'audio-file';
  ...
  const start = useCallback(async () => {
    ...
    console.log(
      'LIVE_TRANSLATION: recording start requested',
      `mode=${captureMode}`,
      `api=${Platform.OS === 'android' ? Platform.Version : Platform.OS}`,
      `preferSavedAudio=${preferSavedAudio}`,
      `liveStt=${!audioOnly}`,
    );

    const granted = await requestSpeechPermissions();
    if (!granted) {
      console.log('LIVE_TRANSLATION: error = microphone/speech permission denied');
      fail('Speech recognition permission is required to transcribe.');
      return null;
    }
    console.log('LIVE_TRANSLATION: microphone permission granted');

    if (!audioOnly && !isSpeechRecognitionAvailable()) {
      console.log('LIVE_TRANSLATION: error = speech recognition unavailable');
      fail(
        'Speech recognition is unavailable on this device. Install Google speech services, or enable “Save audio recording” in Settings to capture audio only.',
      );
      return null;
    }
    ...
    if (fileRecorder) {
      ...
      console.log('LIVE_TRANSLATION: audio-file recorder started (live STT off)');
    }
    ...
    console.log(
      audioOnly
        ? 'LIVE_TRANSLATION: recording started without live text (audio-file mode)'
        : 'LIVE_TRANSLATION: recording started — live speech recognition enabled',
    );
    return true;
  }, [reset, audio, audioOnly, fail, fileRecorder, captureMode, preferSavedAudio]);
