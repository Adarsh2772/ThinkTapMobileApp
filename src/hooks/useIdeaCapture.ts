...
export function useIdeaCapture(
  ...
  /**
   * WHY: expo-audio always records the file, on every Android version and iOS.
   * The take must survive Stop even if the recognizer, the network, or the AI
   * layer fails - losing a captured thought is the highest-severity failure.
   */
  ...
  /**
   * WHY: metering is unreliable across OEMs. Some devices (OPPO A17 and
   * similar) report a constant -160 or 0 no matter what the microphone hears.
   * The silence auto-pause trusted that value, so on those phones a take
   * paused itself every SILENCE_PAUSE_MS whether or not the user was speaking,
   * which read to the user as "recording stops and restarts by itself".
   *
   * Silence auto-pause is therefore opt-in per device: it only arms once we
   * have seen the metering value actually vary. A device that never varies
   * simply never auto-pauses. That is the safe failure - a slightly longer
   * recording beats a take that stops itself mid-thought.
   */
  const meteringSamplesRef = useRef<number[]>([]);
  const meteringUsableRef = useRef(false);

  const noteMeteringSample = useCallback((meter: number) => {
    if (meteringUsableRef.current) return;
    if (typeof meter !== 'number' || !Number.isFinite(meter)) return;
  ...
  // iOS: the recorder owns the mic, so this listens in short bursts for the
  // spoken command only.
  useVoiceStopListener(
    Platform.OS !== 'android' && active && !paused && !stopping,
    () => {
      onVoiceStopRef.current?.();
    },
  );
  ...
  /**
   * WHY this block is gone: it armed a second recogniser to hear "stop" while
   * expo-audio held the microphone. Android refused (ERROR_CLIENT) because two
   * sessions cannot coexist. Voice commands now come from the native capture
   * service, which hears them from the same stream it records - see
   * useNativeCapture.ts.
   */
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      ...
      if (fileRecorder) {
        ...
        noteMeteringSample(meter);
        if (!meteringUsableRef.current) {
          // Device metering unverified - never auto-pause on this phone.
          lastSpeechAtRef.current = Date.now();
          return;
        }
    ...
    return () => clearInterval(id);
  }, [active, paused, stopping, fileRecorder, noteMeteringSample]);
