...
function canPersistRecognitionAudio(): boolean {
   ...
   * version. On API 35 (Android 15) passing recordingOptions terminates the
   * recognition session after ~2 seconds, which is what truncated every take
   * to 2-3 seconds on that version while Android 11 worked fine.
   *
   * expo-audio now always writes the audio file (see captureMode.ts), so this
   * path is retired on every version rather than version-gated. One behaviour
   * everywhere beats three branches.
   */
  return false;
}
